package io.github.yakirchen.watermark.swing.pane;
 ...
 */
public class PDFActionPane extends JPanel {

    public PDFActionPane() {
        super(new GridLayout());
    }

    public PDFActionPane action(PDFTablePane pdfTablePane) {

        var btnSize = new Dimension(80, 20);

        var btnAdd = new JButton("添加PDF");
        btnAdd.addActionListener(PDFChooserListener.bind(btnAdd).datasource(pdfTablePane));
        btnAdd.setPreferredSize(btnSize);
        var btnRemove = new JButton("移除PDF");
        btnRemove.addActionListener(PDFRemoveListener.bind(btnRemove).datasource(pdfTablePane));
        btnRemove.setPreferredSize(btnSize);
        var btnWm = new JButton("为全部PDF添加水印");
        btnWm.setPreferredSize(btnSize);
        btnWm.addActionListener(PDFWatermarkListener.bind(btnWm).datasource(pdfTablePane));
